...
export default function NavMenu() {
  ...
  const unActive =
    ...
    !pathname.startsWith("/settings/products/create");
  return (
    <>
      {unActive && (
        <>
          ...
          <nav className="pretendard w-full  flex justify-center items-center gap-[30px] ">
            {menu.map((item) => (
              <ul className="flex flex-row" key={item.name}>
                <Link href={item.href} className=" text-[16px]  font-medium ">
                  <li
                    className={`flex justify-center items-center w-[280px] h-[48px] border border-[#0089D1] rounded-[9999px] ${
                      isActive(item.href)
